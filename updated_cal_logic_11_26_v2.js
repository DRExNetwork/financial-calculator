...
function computeExitValuesFromEbitda(
    ebitda,
    discountRate = 0.10,
    exitYears = [5, 10, 15]
) {
    /**
     * Exit value = PV at year T of the remaining EBITDA stream (years T+1..n),
     * discounted at 'discount_rate'. Also returns the same exit value discounted
     * to year 0 for apples-to-apples vs CAPEX.
     *
     * ebitda: sequence with index 0..n, where index 0 is pre-ops (often 0 or dummy).
     *         We only use entries from 1..n and, for each exit T, sum T+1..n.
     */
    const e = ebitda;
    const nYears = e.length - 1;
    ...
    for (const T of validExitYears) {
        const remaining = e.slice(T + 1);  // EBITDA in years T+1..n
        ...
        for (let i = 0; i < remaining.length; i++) {
            const k = i + 1;
            exitT += remaining[i] / Math.pow(1.0 + discountRate, k);
        }

        // PV @ T
        const exitT_pv = exitT;

        // PV @ 0 (discount back from year T to year 0)
        const exit0 = exitT / Math.pow(1.0 + discountRate, T);

        rows.push({
            "Exit Year": T,
            "Exit Value @T (USD)": exitT_pv,
            "Exit Value @0 PV (USD)": exit0,
        });
    }
...
async function runCalculation() {
    ...
    const projectCapacityKw = 1000;  // UNIT: kW DC
    const percentageInvestedByOfftaker = 12 / 100;  // UNIT: Convert % to decimal
    const assetOwnershipTransferred = true;  // UNIT: Boolean
    const projectSites = 10;  // UNIT: Number of sites
    const projectCountry = "Colombia";  // UNIT: String
    const epcCostExclVat = 2080000000;  // UNIT: $
    const epcCostVat = 336000;  // UNIT: $
    const currentElectricityTariff = 360;  // UNIT: $/kWh
    const savingOnElectricityTariff = 0 / 100;  // UNIT: Convert % to decimal
    const electricityForecastP90 = 1275000;  // UNIT: kWh
    ...
    for (let ppaTerm = 1; ppaTerm <= 25; ppaTerm++) {
        ...
        if (irr >= targetIrr) {
            ...
            console.log(`Average Annual Output: ${averageAnnualOutput}`);
            console.log(`Equivalent Monthly Payment: ${equivalentMonthlyPayment}`);
            ...
            const ebitdaSeries = optimalResultTable["EBITDA"];

            const exitRecords = computeExitValuesFromEbitda(
                ebitdaSeries,
                0.10,
                [5, 10, 15]
            );

            console.log("exit Records");
    ...
    if (optimalResults) {
        ...
        if (optimalResults.exitRecords.length === 0) {
            ...
            console.log("Exit Year | Exit Value @T ($)    | Exit Value @0 PV ($)");
            console.log("-".repeat(65));
            for (const record of optimalResults.exitRecords) {
                const exitT = formatCurrency(record["Exit Value @T (USD)"]);
                const exit0 = formatCurrency(record["Exit Value @0 PV (USD)"]);
                console.log(
