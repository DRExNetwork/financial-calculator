...
def compute_exit_values_from_ebitda(
    ebitda: list[float] | np.ndarray,
    ...
    """
    e = np.asarray(ebitda, dtype=float)
    n_years = len(e) - 1
    if n_years <= 0:
        return pd.DataFrame(columns=["Exit Value @T (USD)", "Exit Value @0 PV (USD)"])

    valid_exit_years = [T for T in exit_years if 0 < T <= n_years]

    rows = []
    for T in valid_exit_years:
        remaining = e[T + 1 :]  # EBITDA in years T+1..n
        if remaining.size == 0:
            continue
        k = np.arange(1, remaining.size + 1)  # 1..(n-T)
        exit_T = float(np.sum(remaining / (1.0 + discount_rate) ** k))  # PV @ T
        exit_0 = float(exit_T / (1.0 + discount_rate) ** T)  # PV @ 0
        rows.append(
            {
                "Exit Year": T,
                "Exit Value @T (USD)": exit_T,
                "Exit Value @0 PV (USD)": exit_0,
            }
        )

    return pd.DataFrame(rows).set_index("Exit Year")
...
if __name__ == "__main__":
    ...
    target_irr = 12  # UNIT: %                       # PREVIOUSLY 12.25
    ...
    project_es_excluding_vat = 0  # PREVIOUSLY epc_cost_excl_vat * 0.01
    project_due_diligence_excl_vat = (
        0  # PREVIOUSLY fx_table.at[project_country, 1] * 10000
    )
    ...
    es_reporting_excl_vat = 0  # PREVIOUSLY 0.005*epc_cost_excl_vat
    ...
    for ppa_term in range(1, 26):
        ...
        if irr >= target_irr:
            ...
            ebitda_series = result_table.loc["EBITDA"].values.astype(float)

            exit_df = compute_exit_values_from_ebitda(
                ebitda_series,
                discount_rate=0.10,
                exit_years=[5, 10, 15],
            )

            print("\nExit Values DataFrame")
            print(exit_df)

            print("\nExit Values (based on EBITDA)")
            print("==============================================")
            print(exit_df.applymap(lambda x: f"{x:,.2f}"))
            break
